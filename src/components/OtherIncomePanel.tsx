...
function IncomeRow({
  ...
  return (
    <div className={`rounded-xl border p-4 transition-all ${
      hasValue ? 'border-purple-200 bg-purple-50/40' : 'border-gray-100 bg-gray-50/50'
    }`}>
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="flex-1 min-w-48">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-600 mb-1.5">
            ...
            <span>{label}</span>
            {isSpecial && (
              <span className="text-xs bg-orange-100 text-orange-600 font-semibold px-1.5 py-0.5 rounded ml-1">
                Flat rate
              </span>
            )}
          </label>
          <div className="relative">
            ...
            <input
              ...
              className="w-full border border-gray-200 rounded-lg pl-7 pr-3 py-2.5 text-sm font-medium bg-white
                         focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent transition
                         placeholder:text-gray-300"
            />
          </div>
          <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">{info}</p>
          {tdsNote && (
